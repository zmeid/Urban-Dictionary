package com.zmeid.urbandictionary.util

// region WebService
const val BASE_URL = "https://mashape-community-urban-dictionary.p.rapidapi.com/"
const val GET_PATH_DEFINE = "define"
const val DEFINE_QUERY_PARAM_TERM = "term"
const val API_KEY_HEADER_TAG = "X-RapidAPI-Key"
const val API_HOST_HEADER_TAG = "X-RapidAPI-Host"
// endregion

// region SharedPref
const val SHARED_PREF_SHOULD_SORT_BY_THUMBS_UP = "SHARED_PREF_SHOULD_SORT_BY_THUMBS_UP"
// endregion

// region PrefDialog
const val SORTING_PREF_DIALOG_IS_SHOWING_TAG = "isSortingPrefDialogShowing"
// endregion